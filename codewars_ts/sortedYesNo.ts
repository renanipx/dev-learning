...
export function isSortedAndHow(array: number[]): string {
 ...
 for(let i = 0; i < array.length-1 ; i++){
   ...
   if (direction == '') {
     if (next > current) direction = 'ASC'
     if (next < current) direction = 'DESC'
     continue;
   }  
     
   if ((direction == 'ASC')  && (next < current) ) return 'no' 
   if ((direction == 'DESC') && (next > current) ) return 'no' 
 }  

  return direction == 'ASC' ? 'yes, ascending' : 'yes, descending'
}