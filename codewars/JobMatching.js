...
function match(candidate, job) {
  if (!candidate || !job) {
    throw new Error("Need information about candidate and job!");
  }

  if (candidate.minSalary == null) {
    throw new Error("Candidate must have a minimum salary");
  }

  if (job.maxSalary == null) {
    throw new Error("Job must have a maximum salary");
  }
